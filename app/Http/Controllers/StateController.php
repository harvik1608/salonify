    ...
    class StateController extends Controller
    {
            ...
            try {
                ...
                foreach ($rows as $index => $row) {
                    $actions = '<div class="edit-delete-action">';
                        $actions .= '<a href="' . url('admin/states/'.$row->id.'/edit/') . '" class="me-2 edit-icon p-2 text-success" title="Edit">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit">
                                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                            </svg>
                        </a>';
                        $actions .= '<a href="javascript:;" onclick="remove_row(\'' . url('admin/states/' . $row->id) . '\')" data-bs-toggle="modal" data-bs-target="#delete-modal" class="p-2" title="Delete">
                    ...
                    $formattedData[] = [
                        'id' => $start + $index + 1,
                        'name' => $row->name,
                        'country' => $row->country && $row->country->name ? $row->country->name : '',
        ...
        public function create()
        {
            try {
                ...
                return view('admin.state.add_edit',compact('state','countries'));
            } catch (\Exception $e) {
        ...
        public function store(Request $request)
        {
            try {
                ...
                $row = new State;
                $row->name = $post['name'];
                $row->country_id = $post['country_id'];

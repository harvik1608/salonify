...
<form action="{{ is_null($state) ? url('admin/states') : url('admin/states/'.$state->id) }}" method="POST" enctype="multipart/form-data" id="mainForm">
    @csrf
    @if(!is_null($state))
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4>{{ is_null($state) ? "New" : "Edit" }} State</h4>
                </div>
                <div class="card-body profile-body">
                    <div class="row">
                        <div class="col-lg-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="name" id="name" value="{{ is_null($state) ? '' : $state->name }}" autofocus />
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="mb-3">
                                ...
                                <select class="select" name="country_id" id="country_id">
                                    ...
                                    @if($countries)
                                        @foreach($countries as $country)
                                            <option value="{{ $country['id'] }}" {{ !is_null($state) && $state->country_id == $country['id'] ? 'selected' : '' }}>{{ $country['name'] }}</option>
                                        @endforeach
                                    @endif
                                </select>
                        ...
                        <div class="col-lg-12">
                            <div class="mb-3">
                                ...
                                <select class="select" name="is_active" id="is_active">
                                    <option value="1" {{ !is_null($state) && $state->is_active == 1 ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ !is_null($state) && $state->is_active == 0 ? 'selected' : '' }}>Inactive</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="text-end mt-2">
                        ...
                        <a href="{{ url('admin/states') }}" class="btn btn-secondary" id="backBtn">Back</a>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function(){
        $("#mainForm").validate({
            ...
            messages:{
                name:{
                    required: "<small class='text-danger'><b>State Name is required.</b></small>"
                },
                country_id:{
                    required: "<small class='text-danger'><b>Country is required.</b></small>"
                }
            }
        });
